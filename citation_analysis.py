...
def read_in_text(input_text: str) -> List[List[str]]:
    ...
    skipping any empty or whitespace-only lines.
    
    Parameters:
        input_text (str): Path to the input text file.
    
    Returns:
        List[List[str]]: A list containing the non-empty rows of the input file.
    """
    with open(input_text, "r", newline='', encoding='utf-8') as samplefile: 
        ...
        my_list = [
            ...
            if any(cell.strip() for cell in row)  # keeps only non-empty rows
        ]
    return my_list
